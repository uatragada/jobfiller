...
from .document_builder import build_pdf, build_tex, make_cover_letter
...
def _job_output_paths(job: Job) -> tuple[Path, Path, Path, Path]:
    ...
    cover_path = COVER_ROOT / f"{person_slug}-cover-letter-{job_suffix}.md"
...
def generate_artifacts(db: Session, job: Job) -> Artifact:
    ...
    revision_cover_path = cover_dir / f"{person_slug}-cover-letter-{slugify(job.company or 'company')}-rev-{revision:03d}.md"
    revision_cover_path.write_text(make_cover_letter(job), encoding="utf-8")
...
def update_artifact_text(db: Session, artifact: Artifact, kind: str, content: str) -> Artifact:
    ...
    if kind == "latex":
        ...
        if artifact.cover_letter_path and Path(artifact.cover_letter_path).exists():
            _safe_copy(Path(artifact.cover_letter_path), revision_cover_path)
        else:
            revision_cover_path.write_text("", encoding="utf-8")
        compile_status, _ = _ensure_pdf(job, revision_tex_path, revision_pdf_path, resume_dir)
        _safe_copy(revision_pdf_path, pdf_path)
        compile_status = f"{compile_status} (manual latex edit)"
    else:
        revision_cover_path.write_text(content, encoding="utf-8")
        _safe_copy(revision_cover_path, cover_path)
